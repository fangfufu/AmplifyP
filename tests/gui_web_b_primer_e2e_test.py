...
def test_e2e_primer_lifecycle(page: Any, serve_app: str) -> None:
    """Test primer lifecycle: add valid/invalid, and clear state."""
    ...
    # --- 2. ADD INVALID PRIMER (after clean state refresh) ---
    page.goto(f"{serve_app}/?enable-semantics=true")
    expect(page).to_have_title("AmplifyP", timeout=120000)
    wait_for_semantics(page)

    invalid_name = "INVALID_PRIMER"
    invalid_seq = "XYZXYZXYZXYZ"
    add_primer_to_trailing_row(page, invalid_name, invalid_seq)
    ...
    # --- 3. VERIFY STATE CLEARS ON REFRESH ---
    page.goto(f"{serve_app}/?enable-semantics=true")
    expect(page).to_have_title("AmplifyP", timeout=120000)
    wait_for_semantics(page)
    page.wait_for_selector(
    ...
    assert invalid_name not in content
    assert invalid_seq not in content

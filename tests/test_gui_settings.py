...
from amplifyp.settings import Settings, DEFAULT_SETTINGS
...
def test_gui_settings_modification() -> None:
    ...
    # We should probably reset settings after this test or use a fixture if we wanted full isolation,
    # but since this runs in a separate process or we want to test state persistence (as implied by "persist"),
    # we'll follow the script's logic. However, purely modifying global state in tests is risky.
    # For now, I will mirror the script's logic to pass the "verify it persists" requirement 

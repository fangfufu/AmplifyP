...
class GUISettings:
    ...
    def _safe_float(self, key: str, default: float) -> float:
        """Retrieve setting as float, returning default if conversion fails."""
    ...
    def _safe_int(self, key: str, default: int) -> int:
        """Retrieve setting as int, returning default if conversion fails."""
        val = self._settings.get(key)
        if val is None or val == "":
            return default
        try:
    ...
    def load_from_local(self) -> None:
        """Load settings from local YAML file (desktop only)."""
    ...
    def save_to_local(self) -> None:
        """Save settings to local YAML file (desktop only)."""
        config_path = self._get_config_path()
        try:

...
def _get_sha(full: bool = False) -> str:
    ...
    try:
        try:
            from amplifyp.gui2.git_sha import (  # type: ignore[reportMissingImports]
                GIT_FULL_SHA as imported_sha,  # type: ignore[reportMissingImports]
            )
        except ImportError:
            try:
                from amplifyp.gui2.git_sha import (  # type: ignore[reportMissingImports]
                    GIT_SHA as imported_sha,  # type: ignore[reportMissingImports]
                )
            except ImportError:
                imported_sha = None
...
# ==============================================================================
# Lifecycle & Exit Helpers
# ==============================================================================


def confirm_dismiss(controller: Any) -> None:
...
def confirm_exit(controller: Any) -> None:
    """Launch the async window destruction task."""
    controller.confirm_exit_async()


async def capture_view_screenshot_async(
    widget: Any, view_name: str, screenshots_dir: Path | None = None
) -> Path:
    """Capture a PNG screenshot of a widget and save to directory."""
...
async def capture_all_views_async(controller: Any) -> None:
    ...
    try:
        ...
        elif not isinstance(screenshots_dir, (Path, type(None))):
            screenshots_dir = None
        await asyncio.sleep(1.0)
        controller.update_pcr_button_state(sync=True)

        # 1. Input View
        controller.switch_view(controller.input_view)
        await asyncio.sleep(1.0)
        ...
        # 2. PCR View
        controller.switch_view(controller.pcr_view)
        if hasattr(controller, "pcr_view"):
            controller.pcr_view.run_pcr()
            controller.pcr_view.open_all_cards()
        await asyncio.sleep(1.0)
        await capture_view_screenshot_async(
            controller.main_window, "pcr_view", screenshots_dir=screenshots_dir
        )

        # 3. Primer Dimer View
        if hasattr(controller, "dimers_view"):
            controller.dimers_view.run_analysis()
        controller.switch_view(controller.dimers_view)
        await asyncio.sleep(1.0)
        await capture_view_screenshot_async(
            controller.main_window,
            "primer_dimer_view",
...
async def auto_close_and_quit_delayed(
    controller: Any, _event: Any = None
) -> None:
...
def on_window_event(controller: Any) -> bool:
    """Handle window close event, showing close confirmation dialog.

    Returns True if close should be accepted, False to cancel.
    """

...
# Temporarily disable Android toolchain requirement in Flutter config if flutter command is available
$restoreAndroid = $false
if (Get-Command "flutter" -ErrorAction SilentlyContinue) {
    $flutterConfig = flutter config 2>&1 | Out-String
    if ($flutterConfig -notmatch "enable-android:\s*false") {
        $restoreAndroid = $true
...
try {
    ...
    if ($restoreAndroid -and (Get-Command "flutter" -ErrorAction SilentlyContinue)) {
        Write-Host "==> Restoring Flutter Android configuration..."
